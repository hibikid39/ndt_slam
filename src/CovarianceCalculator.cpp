#include "ndt_mapping/CovarianceCalculator.h"
...
double CovarianceCalculator::calIcpCovariance(
    const Pose2D &pose,                   // 推定位置
    std::vector<const LPoint2D*> &curLps, // 現在スキャン
    std::vector<const LPoint2D*> &refLps, // 参照スキャン
    Eigen::Matrix3d &cov)                 // ロボット位置の共分散
{
  double tx = pose.tx;
  double ty = pose.ty;
  double th = pose.th;
  double a = DEG2RAD(th);
  std::vector<double> Jx;                                         // ヤコビ行列のxの列
  std::vector<double> Jy;                                         // ヤコビ行列のyの列
  std::vector<double> Jt;                                         // ヤコビ行列のthの列

  for (size_t i=0; i<curLps.size(); i++) {
    const LPoint2D *clp = curLps[i];                         // 現在スキャンの点
    const LPoint2D *rlp = refLps[i];                         // 参照スキャンの点

    if (clp->type == ISOLATE)               // 孤立点は除外 rlpはすべてLINEに設定済み
      continue;
    if (std::isnan(rlp->nx) || std::isnan(rlp->ny)) // 何故かこれ無いとエラー
      continue;
...
//  printf("[calIcpCovariance]\n");
  // 固有値計算して退化具合を調べる
  double vals[2], vec1[2], vec2[2];
  double ratio = calEigen(cov, vals, vec1, vec2);    // ratio(分散の一方向の広がり具合)
...
//  printf("[DEBUG] (rlp->ny)=%f, (rlp->nx)=%f\n", (rlp->ny), (rlp->nx));

  return(pdis);
}

///////// 運動モデルの計算 /////////
// 1フレーム分の走行によるオドメトリで得た移動量の共分散(簡易版)
void CovarianceCalculator::calMotionCovarianceSimple(
    const Pose2D &motion, // 移動ベクトル
    double dT,            // 時間間隔
    Eigen::Matrix3d &cov) // オドメトリで得た移動量の共分散
{
  ...
  double dx = vt; // 単純に移動距離をx,y方向の並進成分に割り当てる
  double dy = vt;
  double da = wt;

  Eigen::Matrix3d C1;
  C1.setZero();                          // 対角要素だけ入れる
  C1(0,0) = a1_sim*dx*dx;                // 並進成分x
  C1(1,1) = a2_sim*dy*dy;                // 並進成分y
  C1(2,2) = a3_sim*da*da;                // 回転成分
...
double CovarianceCalculator::calEigen(const Eigen::Matrix3d &cov, double *vals, double *vec1, double *vec2) {
  ...
  MyUtil::calEigen2D(cv2, vals, vec1, vec2);        // 固有値分解 vals[0]>vals[1]
